...
const AdminCategoriesPage = () => {
  ...
  const createCategory = async (e) => {
    ...
    try {
      const response = await apiService.adminCreateCategory({
      ...
      dispatch(
        addToast({ type: "error", message: "Failed to create category" }),
      );
    }
  };
  ...
  const saveEditCategory = async (id) => {
    try {
      const response = await apiService.adminUpdateCategory(id, {
        name: editCategoryForm.name,
      });
      if (response.status === "success") {
        setCategories(
          categories.map((c) =>
            c._id === id ? { ...c, name: response.data.name } : c,
          ),
        );
        setEditingCategoryId(null);
  ...
  const deleteCategory = async (id) => {
    ...
    const hasSubs = subcategories.some(
      (s) => s.category?._id === id || s.category === id,
    );
    const msg = hasSubs
  ...
  const createSubcategory = async (e) => {
    ...
    try {
      ...
      dispatch(
        addToast({ type: "error", message: "Failed to create subcategory" }),
      );
    }
  };
  ...
  const saveEditSubcategory = async (id) => {
    try {
      ...
      if (response.status === "success") {
        setSubcategories(
          subcategories.map((s) => (s._id === id ? response.data : s)),
        );
        setEditingSubcategoryId(null);
      ...
      dispatch(
        addToast({ type: "error", message: "Failed to update subcategory" }),
      );
    }
  };
            ...
            <div key={category._id} className="product-card">
              ...
              <div className="card-row">
                ...
                <span className="card-value">
                  {
                    subcategories.filter(
                      (s) => (s.category?._id || s.category) === category._id,
                    ).length
                  }
                </span>
              </div>
                ...
                <div className="action-buttons">
                  {editingSubcategoryId === subcategory._id ? (
                    <>
                      <button
                        className="save-btn"
                        onClick={() => saveEditSubcategory(subcategory._id)}
                      >
                        Save
                      </button>
                      <button
